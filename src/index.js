...
import {
  CLASS_CLOSE_BTN, CLASS_FIRST_STEP_BTN,
  CLASS_NEXT_STEP_BTN,
  CLASS_PREV_STEP_BTN,
  ESC_KEY_CODE,
  ID_POPOVER,
...
export default class Driver {
  ...
  constructor(options = {}) {
    this.options = {
      ...
      materialUse: true,
      singleMode: false,
      materialClass: '.cdk-overlay-container',
      startBtnCallback: () => {},
      nextBtnCallback: () => {},
      prevBtnCallback: () => {},
  ...
  onClick(e) {
    ...
    const materialContainer = this.document.querySelector(this.options.materialClass);
    ...
    if (!clickedHighlightedElement && !clickedPopover && this.options.allowClose) {
      if (this.options.materialUse && materialContainer) {
        if (!materialContainer.contains(e.target)) {
          this.reset();
          return;
        }
      } else {
    ...
    const startClicked = e.target.classList.contains(CLASS_FIRST_STEP_BTN);
    ...
    if (nextClicked) {
      if (this.options.singleMode) {
        this.options.nextBtnCallback();
      } else {
        this.moveNext();
      }
    } else if (prevClicked) {
      if (this.options.singleMode) {
        this.options.prevBtnCallback();
      } else {
        this.movePrevious();
      }
    }
